...
This document defines the schema for all wiki pages in SBOS. Read this when creating or updating pages.
...
|--------|----------------|-------|
| `1 \| Wiki/1.1 \| Network/` | People and companies you interact with | `person`, `company` |
| `1 \| Wiki/1.2 \| Initiatives/` | Projects and tasks, organized by status | `project`, `task` |
| `1 \| Wiki/1.3 \| Knowledge/` | Concepts, frameworks, domain knowledge | `knowledge` |
| `1 \| Wiki/1.4 \| Ledger/` | Decisions and operation logs | `decision`, `log` |
| `1 \| Wiki/1.5 \| Playbooks/` | SOPs, process guides, AI personas | `playbook` |
| `1 \| Wiki/1.6 \| Exports/` | Claude-produced output artefacts | `output` |
| `1 \| Wiki/1.7 \| System/` | Sync scripts, health files, logs | — |
| `1 \| Wiki/1.8 \| Archive/` | Inactive pages kept for reference | — |
...
### Person Page
**Location:** `1 | Wiki/1.1 | Network/[firstname-lastname].md`
...
### Company Page
**Location:** `1 | Wiki/1.1 | Network/[company-name].md`
...
### Project Page
**Location:** `1 | Wiki/1.2 | Initiatives/[status]/[project-name].md`
...
### Task Card
**Location:** `1 | Wiki/1.2 | Initiatives/[status]/[task-title].md`
...
### Knowledge Page
**Location:** `1 | Wiki/1.3 | Knowledge/[topic-name].md`
...
`source_material` is strongly recommended — link to files in `0 | Raw/` where available. Can be left empty for knowledge captured from memory or experience.

### Decision / Log Page
**Location:** `1 | Wiki/1.4 | Ledger/YYYY-MM-DD-decision-name.md`
...
### Playbook Page
**Location:** `1 | Wiki/1.5 | Playbooks/[playbook-name].md`
...
- `sources` should list paths to raw source files in `0 | Raw/`, not URLs
...
Each wiki folder (1.1–1.6) has a folder note that Claude maintains as a living dashboard. These are files named after their parent folder, living inside it (e.g., `1.1 | Network/1.1 | Network.md`).
...
Every page should link to related pages using markdown links: `[Sarah Chen](../1.1 | Network/sarah-chen.md)`
...
`1 | Wiki/hot.md` — ~500-word rolling summary of recent context. Claude reads this first every session. Update after every significant ingest or session.

`1 | Wiki/index.md` — master catalog of all wiki pages, organized by folder. Update whenever pages are created or removed.
...
Every write operation appends to `1 | Wiki/log.md`:

```
## [YYYY-MM-DD] ingest | Source Title — key insight in one line
## [YYYY-MM-DD] update | 1 | Wiki/1.1 | Network/sarah-chen.md — what changed and why
